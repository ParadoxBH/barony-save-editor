...
export function includeEquipamentBag(player?: Player) {
  return !!Object.values(player?.equipment || []).find(
    (slot) => slot.type === ITEMID_BACKPACK
  );
}
...
export const InventoryGrid: React.FC<InventoryGridProps> = ({}) => {
  ...
  const createGrid = () => {
    ...
    Object.values(inventory || []).forEach((item) => {
      const { x, y } = item;
      if (itensInBag(item)) grid[y][x] = item._uuid;
    });
    return grid;
  };
  ...
  function handleDeleteItem() {
    setEditSlot(undefined);
    if (!inventory || editSlot === undefined || editSlot.item === undefined)
      return;
    let newInv = { ...inventory };
  ...
  return (
    <TabWindow label={language.get("tab_inventory")}>
      <Stack spacing={1} alignItems={"center"}>
        {grid.map((row, rowIndex) => (
          <>
            {rowIndex === 6 && (
              <Typography variant="h6" gutterBottom>
                {language.get("tab_inventory_backpack")}
              </Typography>
            )}
                ...
                <Grid size={12 / COLS} key={`${colIndex}-${rowIndex}`}>
                  <Tooltip
                    title={getTooltipTitle(
                      inventory ? inventory[item] || null : null
                    )}
                    arrow
                    disableInteractive
                    placement="right"
